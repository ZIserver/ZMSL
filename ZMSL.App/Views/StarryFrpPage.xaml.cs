...
public sealed partial class StarryFrpPage : Page
{
    public StarryFrpViewModel ViewModel { get; }

    public StarryFrpPage()
    {
        this.InitializeComponent();
        ViewModel = App.Services.GetRequiredService<StarryFrpViewModel>();
    ...
    {
        if ((sender as ListView)?.SelectedItem is StarryFrpProxy proxy)
        {
    ...
    private async void OpenCreateDialog_Click(object sender, RoutedEventArgs e)
    {
        var nodeCombo = new ComboBox
        {
            ...
            DisplayMemberPath = nameof(StarryFrpNode.DisplayName),
        ...
        var remarkBox = new TextBox { Header = "隧道备注", Text = ViewModel.CreateProxyRemark, PlaceholderText = "可选，不填则使用节点名称" };
        ...
        {
            ViewModel.SelectedCreateNode = nodeCombo.SelectedItem as StarryFrpNode;
        ...
        formPanel.Children.Add(remarkBox);
        ...
        if (result == ContentDialogResult.Primary)
        {
            ViewModel.SelectedCreateNode = nodeCombo.SelectedItem as StarryFrpNode;
            ViewModel.CreateProtocol = protocolCombo.SelectedItem?.ToString() ?? "tcp";
            ViewModel.CreateProxyRemark = remarkBox.Text;
